...
DERPFEST_ROOT="${MY_DIR}"/../../..

HELPER="${DERPFEST_ROOT}/vendor/aosip/build/tools/extract_utils.sh"
if [ ! -f "${HELPER}" ]; then
...
setup_vendor "${DEVICE_COMMON}" "${VENDOR}" "${DERPFEST_ROOT}" true "${CLEAN_VENDOR}"
...
if [ -s "${MY_DIR}/../${DEVICE}/proprietary-files.txt" ]; then
    ...
    setup_vendor "${DEVICE}" "${VENDOR}" "${DERPFEST_ROOT}" false "${CLEAN_VENDOR}"
